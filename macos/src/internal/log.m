/**
 * Root Herald native SDK — logging shim (macOS).
 *
 * Single global callback + level filter. NULL callback = silent (default).
 * Symmetrical to the Linux + Windows variants; same public API in
 * <rootherald.h>, same internal rh_log() spelling in log.h.
 */
...
const char* RootHerald_ErrorString(RootHeraldStatus status)
{
    switch (status) {
        ...
        case ROOTHERALD_ERR_TPM_UNAVAILABLE: return "Secure Enclave unavailable on this host";
        ...
        case ROOTHERALD_ERR_ELEVATION_REQUIRED: return "enrollment requires an elevated process; obtain elevation and call RootHerald_RunElevatedEstablishKey, then retry";
