    ...
    RootHeraldClient* client, const char* action, RootHeraldVerifyResult* out_result)
{
    ...
    copy_string(client->deviceId, sizeof(client->deviceId), enroll.device_id);
    /* Linux rootherald doesn't expose RootHeraldSetDeviceId; deviceId is
     * carried in the attest payload internally. */

    /* Attest. */
    RootHeraldAttestationInfo attest;
    memset(&attest, 0, sizeof(attest));
    const char* useAction = (action && action[0]) ? action : "default";
    char synthSession[256];
    snprintf(synthSession, sizeof(synthSession), "rh-verify-%s", useAction);
    const char* synthNonce = "rh-verify-nonce";

    RootHeraldResult ar = RootHeraldAttest(client->endpoint, synthSession,
                                            synthNonce, strlen(synthNonce), &attest);
    ...
    /* Linux is "high" assurance — discrete TPM 2.0. */
